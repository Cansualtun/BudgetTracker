  ...
  const getCategoryTotal = (category: string, type: "income" | "expense") => {
    ...
    if (type === "expense" && category !== "all") {
      ...
      if (categoryObj?.limit) {
        const { percentage } = checkCategoryExpenseLimit(
          transactions,
          categoryObj,
          { showToast: false }
        );
  ...
  useEffect(() => {
    categories.forEach((category) => {
      if (category.type === "expense" && category.limit) {
        const { hasWarning, percentage } = checkCategoryExpenseLimit(
          transactions,
          category,
          { showToast: false }
        );
        if (hasWarning) {
          ...
          toast.error(
            `${t("budget.toast.warning")}: ${t(
              "budget.toast.limitWarning.message",
              {
  ...
  return (
    <div className="mt-8">
        ...
        <nav className="-mb-px flex space-x-4 overflow-x-auto pb-2">
          {allCategories.map((category) => (
                ...
                >
                  ...
                  {category.id !== "all" && (
                    <span className="ml-2 text-xs text-gray-500 dark:text-gray-500">
                      (
                      {
                        transactions.filter((t) => t.category === category.id)
                          .length
                      }
                      )
                    </span>
                ...
                <div className="flex items-center gap-1">
                  <Link
                    href={`/stats/${titleToSlug(category.id)}`}
                    className="flex items-center px-2 py-2 rounded-r-lg border-l hover:bg-secondary/20 transition-colors text-foreground/70 hover:text-foreground border-l-border/20"
                    title={`${category.id} ${t("budget.stats.stats")}`}
                  >
                    <BarChart3 size={18} />
                  </Link>
                  {category.id !== "all" && (
            ...
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {allCategories.find((c) => c.id === transaction.category)
                      ?.label || "Belirtilmemiş"}
                  </span>
                </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <div
          className="bg-green-50 border-green-100 dark:bg-green-500/5 dark:border-green-500/20 
          ...
          <div className="flex items-center gap-2">
            ...
            {getCategoryTotal(activeTab, "expense").warning && (
              <span className="text-xs bg-red-200 dark:bg-red-500/30 text-red-800 dark:text-red-200 px-2 py-1 rounded-full">
                %{getCategoryTotal(activeTab, "expense").percentage.toFixed(1)}{" "}
                limit
              </span>
            )}
          </div>
